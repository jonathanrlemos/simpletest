 ...
 */
#define AT_COLD __attribute__((cold))
 ...
 */
#define AT_CONST __attribute__((const))
 ...
 */
#define AT_MALLOC_LIKE __attribute__((malloc))
 ...
 */
#define AT_PRINTF_LIKE(index) __attribute__((format(printf,index + 1,index + 2)))

/**
 * A function marked with this attribute will always produce the same output given the same input.<br>
 * It also has no side effects.
 */
#define AT_PURE __attribute__((pure))
